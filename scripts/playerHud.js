...
export async function renderPlayerHud() {
  ...
  const hudHtml = await renderFn(
    "modules/stream-director/templates/player-hud.hbs",
    { players }
  );
...
async function renderGMCard() {
  const enabled = game.settings.get("stream-director", "gmCardEnabled");
  if (!enabled) return "";

  const portrait = game.settings.get("stream-director", "gmCardImage");
  const title = game.settings.get("stream-director", "gmCardTitle");
  ...
  return await renderFn(
    "modules/stream-director/templates/gm-card.hbs",
...
async function buildPlayerCard(user) {
  ...
  return await renderFn(
    "modules/stream-director/templates/player-card.hbs",
    data
  );
}
...
Hooks.on("createChatMessage", (msg) => {
  ...
  const duration = game.settings.get("stream-director", "chatBubbleDuration") || 10;
  setTimeout(() => {
...
function playJB2AEffect(card, type) {
  const file = (type === "damage")
    ? game.settings.get("stream-director", "jb2aDamage")
    : game.settings.get("stream-director", "jb2aHeal");
